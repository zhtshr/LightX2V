...
class DecoderService(BaseService):
	def __init__(self, config):
		...
		self.data_mgr = DataManager(data_args, DisaggregationPhase.PHASE2, DisaggregationMode.DECODE)
	...
	def process(self):
		...
		if meta.get("latents_hash") is not None:
			if _sha256_tensor(latents) != meta.get("latents_hash"):
